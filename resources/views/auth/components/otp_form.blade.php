<form class="form" method="POST" action="{{ route('auth.verifyOTP') }}" id="kt_login_forgot_form">
    ...
    <div class="form-group">
        <input class="form-control form-control-solid h-auto py-7 px-6 rounded-lg font-size-h6 d-block text-right @error('otp') is-invalid @enderror" type="text" placeholder="" name="otp" value="{{ old('otp') }}" required  autofocus />
         @error('opt')
